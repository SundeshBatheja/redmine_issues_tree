    ...
    module Helpers
      ...
      class Gantt
        ...
        def render(options={})
            ...
            # --- Custom flat tree logic ---
            children_hash = Hash.new { |h, k| h[k] = [] }
            issues.each { |i| children_hash[i.parent_id] << i }
            roots = children_hash[nil]
            last_project = nil
            begin
                roots.each do |root|
                    current_project = root.project
                    # Render project header if project changed
                    if last_project != current_project
                        # Set indent for the project and render it, passing the main options hash
                        options[:indent] = (current_project.ancestors.count * options[:indent_increment]) + 4
                        render_object_row(current_project, options)
                        last_project = current_project
                    end

                    # Set a base indent for the upcoming issue tree within the options hash
                    options[:issue_base_indent] = ((current_project.ancestors.count + 1) * options[:indent_increment]) + 4
                    
                    # Call the recursive function, which will also use the main options hash
                    render_issue_with_children(root, children_hash, options, 0)
                end
        ...
        def render_issues(issues = self.issues, options = {})
          # Build a hash for quick lookup of children
          children_hash = Hash.new { |h, k| h[k] = [] }
          issues.each { |i| children_hash[i.parent_id] << i }
          ...
          roots.each do |root|
            render_issue_with_children(root, children_hash, options)
          end
        end
  
        def render_issue_with_children(issue, children_hash, options, depth = 0)
            # Store the indent from the parent level to restore it later
            original_indent = options[:indent]
        
            # Calculate and set the indent for the CURRENT issue in the options hash
            base_indent = options[:issue_base_indent] || 24
            options[:indent] = base_indent + (depth * (options[:increment_increment] || 28))
            
            # Render the object row. This will use the new 'indent' and correctly increment 'top'
            # on the persistent options hash.
            render_object_row(issue, options)
        
            # Recurse for all children, passing the same mutable options hash
            children_hash[issue.id].each do |child|
              render_issue_with_children(child, children_hash, options, depth + 1)
            end
            
            # After rendering this issue AND all its descendants, restore the indent.
            # This is critical for ensuring the next sibling item starts at the correct indentation.
            options[:indent] = original_indent
        end
